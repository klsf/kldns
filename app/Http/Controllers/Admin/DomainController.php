...
class DomainController extends Controller
{
    ...
    private function update(Request $request)
    {
        ...
        $recordTypes = Domain::normalizeRecordTypes($request->post('record_types', []));
        $point = abs(intval($request->post('point')));
        $desc = $request->post('desc');
        $reviewMode = intval($request->post('review_mode'));
        $beian = intval($request->post('beian'));

        if (!$did || !$row = Domain::find($did)) {
            $result['message'] = '域名不存在';
        } elseif (empty($groups)) {
            $result['message'] = '请选择用户组';
        } elseif (empty($recordTypes)) {
            $result['message'] = '请选择可用解析类型';
        } else {
            if (in_array('0', $groups)) {
                $groups = ["0"];
            }
            $row->groups = implode(',', $groups);
            $row->record_types = implode(',', $recordTypes);
            $row->review_mode = $reviewMode ? 1 : 0;
            $row->beian = $beian ? 1 : 0;
            $row->point = $point;
            $row->desc = $desc;
            if ($row->save()) {
        ...
        if (!$dns) {
            ...
            if ($list !== false) {
                ...
                if (Domain::where('dns', $dns)->where('domain_id', $domain[0])->first()) {
                    ...
                    'record_types' => implode(',', $recordTypes),
