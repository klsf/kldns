...
class DnsConfigController extends Controller
{
    ...
    private function store(Request $request)
    {
        ...
        if (!$dns) {
            ...
            $row = DnsConfig::find($dns);
            if ($row) {
                $config = array_merge($row->config, array_filter($config, function ($value) {
    ...
    {
        $data = DnsConfig::whereIn('dns', Helper::getList())->orderBy('created_at', 'desc')->pageSelect();
        $data->getCollection()->transform(function (DnsConfig $row) {
            $row->makeHidden(['config']);
            return $row;
        });
        return ['status' => 0, 'message' => '', 'data' => $data];
    }
